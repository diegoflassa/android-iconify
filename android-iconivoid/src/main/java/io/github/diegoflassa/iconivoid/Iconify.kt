...
import io.github.diegoflassa.iconivoid.internal.IconFontDescriptorWrapper
import io.github.diegoflassa.iconivoid.internal.ParsingUtil.parse
import java.util.*
...
object IconiVoid {
     ...
     */
    fun with(iconFontDescriptor: IconFontDescriptor): IconiVoidInitializer {
        return IconiVoidInitializer(iconFontDescriptor)
    }
    ...
    private fun addIconFontDescriptor(iconFontDescriptor: IconFontDescriptor) {
        ...
        for (wrapper in iconFontDescriptors) {
            if (wrapper.iconFontDescriptor.ttfFileName() == iconFontDescriptor.ttfFileName()
            ) {
                return
            }
        }
    ...
    /**
     * Allows chain calls on [IconiVoid.with].
     */
    class IconiVoidInitializer(iconFontDescriptor: IconFontDescriptor) {
        /**
         * Add support for a new icon font.
         * @param iconFontDescriptor The IconDescriptor holding the ttf file reference and its mappings.
         * @return An initializer instance for chain calls.
